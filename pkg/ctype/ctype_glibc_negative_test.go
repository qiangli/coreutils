...
//go:build linux && (amd64 || arm64) && !bashy_scratch
...
func TestLoadLibcNegative(t *testing.T) {
	...
	t.Run("missing_gnu_get_libc_version", func(t *testing.T) {
		closed := 0
		ops := libcLoaderOps{
			open: func(string, int) (uintptr, error) { return fakeHandle, nil },
			sym: func(_ uintptr, name string) (uintptr, error) {
				if name == "gnu_get_libc_version" {
					return 0, errors.New("not glibc")
				}
				return 1, nil
			},
			close: func(handle uintptr) error {
		...
		if _, err := loadLibcWith("fake", ops); !errors.Is(err, ErrGlibcUnavailable) {
			t.Errorf("loadLibcWith(not glibc) = %v, want ErrGlibcUnavailable", err)
		}
		if closed != 1 {
			t.Errorf("close calls = %d, want exactly 1", closed)
		}
...
func TestVerifyCodesetNegative(t *testing.T) {
	...
	tests := []struct {
		...
		{"wrong_codeset_for_iso88591", wrongCodeset, []string{codesetISO88591, codesetISO88591Alt}},
		{"wrong_codeset_for_c", wrongCodeset, []string{codesetC}},
		{"unterminated", unterminated, []string{codesetISO88591, codesetISO88591Alt}},
		{"overlong", overlong, []string{codesetISO88591, codesetISO88591Alt}},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := b.verifyCodeset(1, tc.want); !errors.Is(err, ErrCodeset) {
