...
func (TerminalRenderer) Render(b *Board, opts Options) ([]byte, error) {
	...
	fmt.Fprintf(&out, "%d todos · %d sprints · %d runs · %d need steward · ETA median %s\n", b.Summary.Todos, b.Summary.Sprints, b.Summary.Runs, b.Summary.NeedsSteward, duration(b.Summary.ETAMedianSeconds))
	for _, lane := range b.Lanes {
		...
		for _, c := range lane.Cards {
			switch c.Layer {
			case "run":
				fmt.Fprintf(&out, "  #%s %-28s %-10s %-3s %-20s elapsed %s  eta %s\n", c.ID, c.Label, dash(c.Tool), band(c.Band), dash(c.Model), duration(c.Elapsed), duration(c.ETA))
			default:
