	...
	for _, lane := range b.Lanes {
		...
		for _, c := range lane.Cards {
			...
			case "run":
				...
				fmt.Fprintf(&out, "  #%s %-28s %-10s %-3s %-20s elapsed %s  eta %s%s\n", c.ID, c.Label, dash(c.Tool), band(c.Band), dash(c.Model), duration(c.Elapsed), duration(c.ETA), salvage)
			default:
