...
func TestEnvelopeErrorFromDetail(t *testing.T) {
	// text is the rendered err.Error(); for a direct foreign error that is
	// sh's own rendering ("Code: message[: cause…]"), for a wrapped one the
	// caller's context comes first. Message is always that text; the worker's
	// own message only stands in when there is no rendered text at all.
	...
	cases := []struct {
	...
	}{
		{
			...
			want:  EnvelopeError{Code: "ValueError", Message: "ValueError: boom", Help: "Traceback (most recent call last):\n  raise ValueError('boom')"},
		},
		{
			name:  "structured rust result (serde_json test_missing_nonoption_field)",
			frame: `{"code":"RUST-ECALL","message":"missing field ` + "`x`" + `"}`,
			text:  "RUST-ECALL: missing field `x`",
			code:  ExitGenericFail,
			want:  EnvelopeError{Code: "RUST-ECALL", Message: "RUST-ECALL: missing field `x`"},
		},
		{
			name:  "wrapped structured error keeps the wrapper context in the message",
			frame: `{"code":"ValueError","message":"boom","help":"h"}`,
			text:  "island call: ValueError: boom",
			code:  ExitGenericFail,
			want:  EnvelopeError{Code: "ValueError", Message: "island call: ValueError: boom", Help: "h"},
		},
		{
		...
		{
			name:  "wrapped legacy plain string keeps the wrapper context",
			frame: `"plain boom"`,
			text:  "island call: plain boom",
			code:  ExitPrecondFail,
			want:  EnvelopeError{Code: "precondition_failed", Message: "island call: plain boom"},
		},
		{
			name:  "missing message: the rendered text is the message",
			frame: `{"code":"KeyError"}`,
			text:  "KeyError",
			code:  ExitGenericFail,
			want:  EnvelopeError{Code: "KeyError", Message: "KeyError"},
		},
		{
		...
		{
			name:  "no rendered text: the worker message stands in",
			frame: `{"code":"ValueError","message":"boom"}`,
			text:  "",
			code:  ExitGenericFail,
			want:  EnvelopeError{Code: "ValueError", Message: "boom"},
		},
		{
			name:  "nested cause maps field for field under the rendered message",
			frame: `{"code":"RuntimeError","message":"outer","cause":{"code":"ValueError","message":"inner","help":"inner trace"}}`,
			text:  "RuntimeError: outer: ValueError: inner",
			code:  ExitGenericFail,
			want: EnvelopeError{Code: "RuntimeError", Message: "RuntimeError: outer: ValueError: inner",
		...
		},
		{
			...
			text:  "RuntimeError: outer",
			code:  ExitGenericFail,
			want:  EnvelopeError{Code: "RuntimeError", Message: "RuntimeError: outer", Cause: &EnvelopeError{}},
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			got := EnvelopeErrorFromDetail(tc.code, detail, tc.text)
			if got == nil {
...
func TestEnvelopeError_JSONCompatibility(t *testing.T) {
	...
	structured := EnvelopeErrorFromDetail(ExitGenericFail, decodeDetail(t,
		`{"code":"RuntimeError","message":"outer","help":"trace","cause":{"code":"ValueError","message":"inner"}}`),
		"RuntimeError: outer: ValueError: inner")
	...
	if legacy.Error == nil || legacy.Error.Code != "RuntimeError" || legacy.Error.Message != "RuntimeError: outer: ValueError: inner" {
