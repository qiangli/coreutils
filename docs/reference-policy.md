...
| Other standardized Go applets, including `awk`, `diff`, `ed`, `grep`, `mailx`, `sed`, and `talk` | POSIX.1 Issue 7, 2016 Edition | The official documentation and explicitly recorded version of the command's actual upstream family only for extensions the applet claims to support; local-only mailx/talk and other incomplete surfaces remain explicitly partial |
| Active pinned external providers: `make`, `bc`, `m4`, `man`, `ctags`, `ar`, `nm`, `strip`, `ex`, `vi`, `lp`, and `localedef` | POSIX.1 Issue 7, 2016 Edition | The exact upstream project, version, artifact, and digest in [`pkg/posixprovider/manifest.tsv`](../pkg/posixprovider/manifest.tsv); retained `ed`, `mailx`, `patch`, and `talk` rows are differential controls, not runtime provider owners |
