...
| Active pinned external providers: `make`, `bc`, `m4`, `man`, `ctags`, `ar`, `nm`, `strip`, `ex`, `vi`, `lp`, and `localedef` | POSIX.1 Issue 7, 2016 Edition | The exact upstream project, version, artifact, and digest in [`pkg/posixprovider/manifest.tsv`](../pkg/posixprovider/manifest.tsv); `ed`, `patch`, `mail`/`mailx`, and `talk` are exclusively pure-Go applets |
