...
- [uuencode-stdin-and-zero-sextet-framing](pages/uuencode-stdin-and-zero-sextet-framing.md) `validated/lesson` uuencode stdin and zero sextet framing — For POSIX uuencode, omitted input uses a 0644 header mode and '-' is a pathname; classic zero sextets and the terminating zero-length line use SPACE, while uudecode may accept backtick. Preserve exact decoded modes with a post-write chmod instead of changing process umask.
