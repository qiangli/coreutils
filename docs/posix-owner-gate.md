...
- a registered **Bashy Go applet** (87 names),
- the **shell** (14 names: the `sh` entry point plus builtins), or
- one of the fifteen **active pinned POSIX external providers** the multicall
  registers and dispatches (see
  [posix-external-providers.md](posix-external-providers.md)).

That is the **availability** split, 87/14/15. What a POSIX-mode shell
...
79/22/15. The gate pins both, explicitly, and verifies the effective split
...
| Rejection | Meaning |
| --- | --- |
| count drift | availability no longer splits 116 = 87/14/15, **or** effective selection no longer splits 79/22/15; both axes are hard pins that must be changed deliberately, in one reviewed place |
...
staged certification runtime that cannot supply all fifteen active names is not the
...
3. **PATH ownership + identity of every multicall-owned name** — every
   multicall-owned name (87 applets + 15 providers) and `sh` itself must
...
parsed as strictly as the classification transcript (exactly fifteen rows,
...
entire 87/14/15 → 79/22/15 difference; the effective pins mean an eighth
