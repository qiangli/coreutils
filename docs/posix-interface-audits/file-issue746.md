...
| --- | --- | --- |
| Required synopsis and operands | `cmds/file/file.go:33`, `cmds/file/file.go:82-100` register `-b`, `-d`, `-h`, `-i`, `-M`, `-m` and reject missing operands or `-i` with magic/default options. | `cmds/file/issue7_test.go:76`, `cmds/file/file_test.go:245`, `cmds/file/issue7_test.go:103` |
| `-b` output | `cmds/file/file.go:125-130` omits the prefix for successful classifications, but keeps the operand name for inaccessible/undetermined results. | `cmds/file/file_test.go:101`, `cmds/file/file_unix_test.go:14`, `cmds/file/issue7_test.go:88` |
...
| Exact stdout shape and operand ordering | `cmds/file/file.go:108-136` emits exactly one line per operand in operand order, `<operand>: <type>\n` unless successful `-b` applies. | `cmds/file/issue7_test.go:26`, `cmds/file/issue7_test.go:88`, `cmds/file/file_test.go:790` |
...
- Fixed a POSIX stdout-shape defect where `file -b missing` omitted the operand name for an inaccessible operand. The implementation now keeps `-b` brief output only for successful classifications and forces the `<operand>: cannot open ...` shape for inaccessible or undetermined operands.
- Added command-local executable tests for inaccessible `-b` output, attached and separate `-m`/`-M` option arguments, repeated and interspersed source-order precedence, and supported `--` parsing.
