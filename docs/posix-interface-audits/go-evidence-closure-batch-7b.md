...
| `tr` | [Issue 7 tr](https://pubs.opengroup.org/onlinepubs/9699919799.2016edition/utilities/tr.html) | partial | All four required forms, `-c`/`-C`/`-d`/`-s`, escapes, octal values, ranges, repetitions, equivalence classes, required character classes, case conversion, complement/delete/squeeze semantics, stdin read failures, stdout/EPIPE handling, diagnostics, and status are covered for C/POSIX and injected single-byte tables. A whole-engine multibyte character/collation model for non-C locales and translated diagnostics remain absent. |
