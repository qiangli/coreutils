...
| `csplit` | [Issue 7 csplit](https://pubs.opengroup.org/onlinepubs/9699919799.2016edition/utilities/csplit.html) | partial | Locale wave A routes BRE classes, equivalence, and collation ranges through invocation-owned carried providers, preserves multibyte occurrence stepping, and fails closed before input for unsupported products. The `-k` cleanup path is covered through a filesystem write failure. The provider/platform corpus remains bounded; GNU extensions beyond `[-ks] [-f prefix] [-n number]` remain outside certification. |
...
| `expand` | [Issue 7 expand](https://pubs.opengroup.org/onlinepubs/9699919799.2016edition/utilities/expand.html) | partial | Accepted source retains exact byte spans and applies invocation-owned `LC_CTYPE` to display-column accounting. C/POSIX, UTF-8 widths, ISO-8859-1, malformed input, precedence, `-i`, read errors, and short writes have focused evidence. The carried locale corpus and Unicode width policy remain bounded; `-i` and `-U` remain GNU extensions beyond `[-t tablist]`. |
