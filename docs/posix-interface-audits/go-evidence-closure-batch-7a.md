...
| `ps` | [XCU ps](https://pubs.opengroup.org/onlinepubs/9699919799.2016edition/utilities/ps.html) | partial | All XSI option spellings are accepted. Wave 7A fixes selection-option union, uses the permitted `-a` session-leader omission, selects `-g` by session ID, accepts blank- or comma-separated selection lists, implements `-o` header text containing blanks and null-header field width, defaults `args` to `COMMAND`, selects by the invoking effective user and terminal by default, distinguishes Linux real/effective UID/GID, accepts proc-backed `-n`, and propagates stdout errors. `TestPSPOSIXSelectionUnionAndDefaults`, `TestPSPOSIXListSeparatorsAndFormatHeaders`, and `TestPSPOSIXNameListAcceptedAndOutputErrorsFail` pin the corrections. | Non-Linux process enrichment cannot currently distinguish real/effective IDs, controlling terminal, groups, session/process group, CPU, memory, state, or niceness because the portable enumerator lacks those values. On Linux, `F`, `S`, `PRI`, `ADDR`, `WCHAN`, and `%CPU` are placeholders rather than complete POSIX field values. `COLUMNS` width behavior, non-C `LC_TIME`, translated user/group/diagnostic data, and a hermetic process-enumerator seam remain residual. |
