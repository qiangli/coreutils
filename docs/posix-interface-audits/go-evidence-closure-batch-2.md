...
| `date` | [Issue 7 date](https://pubs.opengroup.org/onlinepubs/9699919799.2016edition/utilities/date.html) | partial | Translated diagnostics absent. The XSI set-date form `date [-u] mmddhhmm[[cc]yy]` is recognized and refused loudly (exit 2, nothing on stdout) — a documented not-supported interface, so XSI closure is impossible by design. LC_TIME is neither C-locale-only nor generally implemented: a single hard-coded `de_DE` table localizes `%a %A %b %B` and the names inside `%c`, while `%c`'s layout, `%x`, `%X`, `%r`, and `%p` stay C-locale and every other locale silently degrades to C. `%C`/`%y` can leave [00,99] via the non-POSIX `-d @epoch` path; `%S` can never be 60 (Go time cannot represent a leap second); `%h` is implemented but untested. |
