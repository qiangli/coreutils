...
| `ls` | [Issue 7 ls](https://pubs.opengroup.org/onlinepubs/9699919799.2016edition/utilities/ls.html) | partial | The full POSIX option set, `--`, POSIX block sizing, and Issue 777's last-`-H`/`-L` ordering across clusters/spellings are covered. A sticky checked writer propagates immediate errors, nil-error short writes, and late failures across every listing format, help/version, continuation, and recursion. Remaining: non-C `LC_COLLATE`/`LC_TIME`, terminal width/capability discovery, and non-Unix runtime metadata behavior. |
