...
// This platform has no POSIX uid/gid ownership: there is no user or group
// for a file to belong to, so there is nothing an ownership change could do
// and nothing it could undo. chown is therefore a SUCCESSFUL NO-OP here — it
...
// test.tests changes the group of a scratch file and then tests -g on it;
// a hard error
...
// `chown u:g missing` still fails with the GNU diagnostic and exit 1. What
...
func apply(rc *tool.RunContext, _ string, o options) int {
	...
	for _, name := range o.files {
		if _, err := os.Lstat(rc.Path(name)); err != nil {
			...
			if !o.silent {
				fmt.Fprintf(rc.Err, "chown: cannot access '%s': %s\n", name, tool.SysErrString(err))
			}
			exit = 1
		}
	}
...
// parseSpec accepts any OWNER[:GROUP] spec: there are no ids to resolve it
// to, and nothing to compare it against.
