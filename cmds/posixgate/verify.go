	...
	for _, n := range providerNames {
		...
		if !specProviders[n] {
			out = append(out, Finding{Check: "provider-set", Name: n,
				Detail: "active in provider dispatch but not external_provider in the inventory"})
		}
	}
...
// certification runtime that cannot supply all ten active names is not the
...
// active pinned provider, ten in total. Duplicates, extras, missing names,
...
func parseDispatchPlan(plan string) (map[string]planRow, []Finding) {
	...
	for i, line := range strings.Split(strings.TrimSuffix(plan, "\n"), "\n") {
		...
		case !posixprovider.IsDispatchProvider(f[0]):
			out = append(out, Finding{Check: "provider-dispatch", Name: f[0],
				Detail: "dispatch-plan row for a name outside the ten active providers"})
		default:
	...
	if len(rows) != pinProviders {
		out = append(out, Finding{Check: "provider-dispatch",
			Detail: fmt.Sprintf("dispatch plan accounts for %d active providers, want exactly %d", len(rows), pinProviders)})
	}
	return rows, out
}
