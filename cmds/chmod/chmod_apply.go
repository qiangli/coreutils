...
// credentials or unsafe mounts. On Windows os.Chmod sets or clears
// FILE_ATTRIBUTE_READONLY from the owner write bit; hostMode (below) is
...
// hostMode is the os.FileMode handed to changeMode for the computed POSIX
...
func hostMode(bits uint32) os.FileMode {
	...
	return readOnlyProjection(bits)
}

// readOnlyProjection is the Windows mapping in hostMode, kept as a pure
// function so the rule is tested on every host: any of the three write
// bits keeps the file writable (0666, which os.Chmod turns into "clear
// FILE_ATTRIBUTE_READONLY"); none of them sets it (0444).
func readOnlyProjection(bits uint32) os.FileMode {
	if bits&0o222 != 0 {
		return 0o666
	}
	return 0o444
