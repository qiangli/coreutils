...
// sha1("abc") — FIPS 180 test vector.
const abcDigest = "a9993e364706816aba3e25717850c26c9cd0d89d"
...
func TestCheck(t *testing.T) {
	...
	if out != "f.txt: FAILED\n" || code != 1 ||
		!strings.Contains(errb, "sha1sum: WARNING: 1 computed checksum did NOT match") {
...
func TestErrors(t *testing.T) {
	...
	if code != 1 || !strings.Contains(errb, "sha1sum: nope.txt: No such file or directory") {
...
func TestHelpAndVersion(t *testing.T) {
	...
	if code != 0 || !strings.Contains(out, "Usage: sha1sum") {
		t.Errorf("--help: code=%d out=%q", code, out)
	}
	out, _, code = runTool(t, "", "", "--version")
	if code != 0 || !strings.Contains(out, "sha1sum") {
