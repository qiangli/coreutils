...
# Availability vs effective selection. The names below are AVAILABLE as Go
# applets (they are on the staged PATH for exec-style callers: env, xargs,
...
# canonical manifest.
BUILTIN_OVERLAP = set("echo false kill printf pwd test true".split())
KEYWORD_OVERLAP = {"time"}
...
def effective_selection(command: str, disposition: str) -> str:
    """What a POSIX-mode bash-family shell actually selects for a name."""
    if disposition == "external_provider":
        return "external_provider"
    if command == "sh":
        return "shell_entry"
    if disposition == "shell" or command in BUILTIN_OVERLAP:
        return "shell_builtin"
    if command in KEYWORD_OVERLAP:
        return "shell_keyword"
    return "go_applet"


def effective_counts(data: list[dict[str, str]]) -> dict[str, int]:
    counts = {"go_applet": 0, "shell": 0, "external_provider": 0}
    for row in data:
        eff = effective_selection(str(row["command"]), str(row["profile_cd_disposition"]))
        if eff in ("shell_entry", "shell_builtin", "shell_keyword"):
            counts["shell"] += 1
        else:
            counts[eff] += 1
    return counts
...
# The Go identifiers spec_gen.go uses for each projection value.
GO_OWNER = {
    "go_applet": "OwnerGoApplet",
    "shell": "OwnerShell",
    "external_provider": "OwnerProvider",
}
GO_SELECTOR = {
    "go_applet": "SelGoApplet",
    "shell_entry": "SelShellEntry",
    "shell_builtin": "SelShellBuiltin",
    "shell_keyword": "SelShellKeyword",
    "external_provider": "SelProvider",
}


def render_spec_gen(data: list[dict[str, str]]) -> str:
    lines = [
    ...
    for row in data:
        disposition = str(row["profile_cd_disposition"])
        eff = effective_selection(str(row["command"]), disposition)
        lines.append(
            "\t{Command: %s, GoPackage: %s, Owner: %s, Effective: %s},"
            % (
                go_quote(str(row["command"])),
                go_quote(str(row["go_package"])),
                GO_OWNER[disposition],
                GO_SELECTOR[eff],
            )
        )
    lines.extend(["}", ""])
...
def go_quote(text: str) -> str:
    if any(ch in text for ch in '\\"\n\t'):
        raise SystemExit(f"cannot safely quote {text!r} into Go source")
    return f'"{text}"'
...
def render_required_markdown(data: list[dict[str, str]]) -> str:
    ...
    effective = effective_counts(data)
    lines = [
        ...
        "| --- | ---: | ---: | ---: |",
        f"| Availability | {go_count} | {shell_count} | {provider_count} |",
        f"| Effective selection | {effective['go_applet']} | {effective['shell']} | {effective['external_provider']} |",
        "",
...
def main() -> None:
    ...
    if counts != AVAILABILITY_PIN:
        raise SystemExit(f"required availability changed: {counts}; reconcile before regenerating")
    if (not BUILTIN_OVERLAP <= POSIX_CERT_REQUIRED or not KEYWORD_OVERLAP <= POSIX_CERT_REQUIRED
            or BUILTIN_OVERLAP & SHELL_PROVIDED or KEYWORD_OVERLAP & SHELL_PROVIDED):
        raise SystemExit("builtin/keyword overlap sets are inconsistent with the required inventory")
    if effective_counts(required) != EFFECTIVE_PIN:
        raise SystemExit(f"effective selection changed: {effective_counts(required)}; reconcile before regenerating")
    ...
    # cmds/posixgate consumes a GENERATED Go projection of the canonical
    # manifest (availability owner + effective selector per name) rather than
    # maintaining any copy of the inventory — a hand edit to the projection or
    # to the manifest fails --check here.
    spec_gen = ROOT / "cmds/posixgate/spec_gen.go"
    ...
    if args.check:
        if (not tsv.exists() or not markdown.exists() or
                ...
                spec_gen.read_text() != rendered_spec_gen):
    ...
    spec_gen.write_text(rendered_spec_gen)
