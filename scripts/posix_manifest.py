...
CUSTOM_PARSERS = frozenset("awk dd expr find make sed stty".split())
    ...
    for item, option in declared_option_arguments(row).items():
        ...
        manual_value = (
            ...
            and (
                ...
                or "requires an operand" in source
                or "val()" in source
        ...
        manual_char_value = (
            re.search(rf"case\s+[^:\n]*'{short}'[^:\n]*:", source)
            and ("requires an argument" in source or "requires an operand" in source)
        )
