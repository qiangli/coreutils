...
func (p *interp) execute(code []compiler.Opcode) error {
	for ip := 0; ip < len(code); {
		...
		case compiler.Str:
			index := code[ip]
			ip++
			p.push(localeStr(p.strs[index], p.decimalPoint))
		...
		case compiler.IndexMulti:
			...
			p.push(localeStr(strings.Join(indices, p.subscriptSep), p.decimalPoint))
		...
		case compiler.Less:
			...
			rn, rIsStr := r.isTrueStr()
			if lIsStr || rIsStr {
				cmp, err := p.compareStrings(p.toString(l), p.toString(r))
				if err != nil {
					return err
				}
				p.replaceTop(boolean(cmp < 0))
			} else {
				p.replaceTop(boolean(ln < rn))
			}
		...
		case compiler.Concat:
			l, r := p.peekPop()
			p.replaceTop(localeStr(p.toString(l)+p.toString(r), p.decimalPoint))
		...
		case compiler.ConcatMulti:
			...
			p.push(localeStr(sb.String(), p.decimalPoint))
		...
		case compiler.ForIn:
			...
			for index := range array {
				...
				case resolver.Global:
					p.globals[varIndex] = localeStr(index, p.decimalPoint)
				case resolver.Local:
					p.frame[varIndex] = localeStr(index, p.decimalPoint)
				default: // resolver.Special
					err := p.setSpecial(int(varIndex), localeStr(index, p.decimalPoint))
					if err != nil {
						return err
					}
				}
		...
		case compiler.CallSprintf:
			...
			p.push(localeStr(s, p.decimalPoint))
		...
		case compiler.GetlineGlobal:
			...
			if ret == 1 {
				p.globals[index] = localeNumStr(line, p.decimalPoint)
			}
			p.push(num(ret))
		...
		case compiler.GetlineLocal:
			...
			if ret == 1 {
				p.frame[index] = localeNumStr(line, p.decimalPoint)
			}
			p.push(num(ret))
		...
		case compiler.GetlineSpecial:
			...
			if ret == 1 {
				err := p.setSpecial(int(index), localeNumStr(line, p.decimalPoint))
				if err != nil {
					return err
				}
			}
		...
		case compiler.GetlineArray:
			...
			if ret == 1 {
				array := p.array(resolver.Scope(arrayScope), int(arrayIndex))
				array[index] = localeNumStr(line, p.decimalPoint)
			}
...
func (p *interp) callBuiltin(builtinOp compiler.BuiltinOp) error {
	...
	case compiler.BuiltinGsub:
		...
		p.replaceTwo(num(float64(n)), localeStr(out, p.decimalPoint))
	...
	case compiler.BuiltinSubstr:
		...
		p.replaceTop(localeStr(substr, p.decimalPoint))
	...
	case compiler.BuiltinTolower:
		mapped, err := p.toLower(p.toString(p.peekTop()))
		if err != nil {
			return err
		}
		p.replaceTop(localeStr(mapped, p.decimalPoint))

	case compiler.BuiltinToupper:
		mapped, err := p.toUpper(p.toString(p.peekTop()))
		if err != nil {
			return err
		}
		p.replaceTop(localeStr(mapped, p.decimalPoint))
...
func (p *interp) compareStrings(a, b string) (int, error) {
	if p.stringCompare != nil {
		return p.stringCompare(a, b)
	}
