...
import (
	...
	ociJaeger "github.com/qiangli/coreutils/external/otel/jaeger"
)
...
type JaegerComponent struct {
	...
	queryGRPCPort int // Jaeger Query gRPC port
	dataDir       string
...
func NewJaegerComponent(otlpPort, queryPort int, dataDir string) *JaegerComponent {
	return &JaegerComponent{otlpPort: otlpPort, queryPort: queryPort, queryGRPCPort: 16685, dataDir: dataDir}
}

func NewJaegerComponentWithQueryGRPC(otlpPort, queryPort, queryGRPCPort int, dataDir string) *JaegerComponent {
	return &JaegerComponent{otlpPort: otlpPort, queryPort: queryPort, queryGRPCPort: queryGRPCPort, dataDir: dataDir}
}
...
func (j *JaegerComponent) Start(ctx context.Context) error {
	...
	settings := ociJaeger.CollectorSettings{
		BuildInfo: ociJaeger.BuildInfo{
			Command:     "otel-jaeger",
			Description: "Embedded Jaeger for OTEL",
			Version:     "0.1.0",
		},
...
`, j.otlpPort, basePath, j.queryPort, j.queryGRPCPort)
}
